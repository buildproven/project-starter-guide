...
export default async function globalSetup() {
  ...
  if (fs.existsSync(lockFile)) {
    ...
    try {
      ...
      throw new Error(`Integration tests already running (PID: ${lockPid}). Wait for completion or remove /tmp/api-service-test.lock`);
    } catch (error) {
      // Process doesn't exist, remove stale lock
      console.log(`🧹 Removing stale lock file (PID ${lockPid} no longer exists)`);
  ...
  console.log("✅ Test database setup complete (using temp schema, tracked files untouched)");
}
